...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.send = function(message, cb) {
            ...
            //Skip Facebook webhook if whatsApp channel
            if (message) if(message.recipient) if(message.recipient.id) {
                if (message.recipient.id.includes('whatsapp')) {
                    dummyBody = { recipient_id: '1089098164520058',
                    message_id: 'm_xJBDs3VBwsegoW1uxDrLQVKdFFR' }
                    return cb && cb(null, dummyBody);
                }
            }
